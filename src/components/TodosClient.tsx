...
interface TodosClientProps {
  todoMachines: MachineWithData[];
}

const MACHINES_PER_PAGE = 12;

export default function TodosClient({ todoMachines }: TodosClientProps) {
  const [currentPage, setCurrentPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState("");

  // Filter machines based on search
  const filteredMachines = useMemo(() => {
    return todoMachines.filter((item) => {
      ...
      <div className="relative z-10 max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-green-400 font-mono">
            Machines you want to work on
            <span className="ml-2">
              <Badge variant="success">{filteredMachines.length} todos</Badge>
            </span>
          </p>
        </div>
        ...
        {todoMachines.length === 0 ? (
          <div className="text-center py-12">
            <TerminalWindow title="todos@tasave:~$" showControls>
              <div className="space-y-2">
                <div className="text-green-400">
                  <CheckSquare className="w-8 h-8 mx-auto mb-4" />
                </div>
                ...
                <div className="flex">
                  ...
                  <span>cd /dashboard && add_todos</span>
                </div>
              </div>
            </TerminalWindow>
          ...
          <>
            ...
            {paginatedMachines.length === 0 ? (
              <div className="text-center py-12">
                <TerminalWindow title="search@todos:~$" showControls>
                  <div className="space-y-2">
                    <div className="text-yellow-400">
                      [WARNING] No todos match your search
                    </div>
                    <div className="text-gray-400">
              ...
              <>
                ...
                <div className="mt-8 text-center">
                  <p className="text-sm text-gray-400 font-mono">
                    Showing {startIndex + 1}-{Math.min(startIndex + MACHINES_PER_PAGE, filteredMachines.length)} of {filteredMachines.length} todos
                    {filteredMachines.length !== todoMachines.length && (
                      <span className="ml-2">
                        (filtered from {todoMachines.length} total)
                      </span>
                    )}
                  </p>
                </div>
